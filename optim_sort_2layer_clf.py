...
def score_GNB(X, y):
    return accuracy_score(y, GaussianNB().fit(X, y).predict(X))

def score_KNN(X, y):
    return accuracy_score(y, KNeighborsClassifier().fit(X, y).predict(X))
...
n_features = 2
n_features_target = 2
X, y = make_blobs(n_samples=500, n_features=n_features, centers=4)
y = y%2
